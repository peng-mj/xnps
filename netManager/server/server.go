...
import (
	...
	proxy2 "xnps/netManager/server/proxy"
	...
	for {
		...
		case s := <-Bridge.SecretChan:
			...
			if t := Mapper.GetDb().GetTunnelByMd5Password(s.Password); t != nil {
				if t.Status {
					go proxy2.NewBaseServer(Bridge, t).DealClient(s.Conn, t.Client, t.Target.TargetStr, nil, common.CONN_TCP, nil, t.Target.LocalProxy, nil)
				} else {
					s.Conn.Close()
...
func NewMode(Bridge *bridge.Bridge, c *models.Tunnel) proxy2.Service {
	var service proxy2.Service
	switch c.Mode {
	case "tcp", "file": //这里需要修改随机
		service = proxy2.NewTunnelModeServer(proxy2.ProcessTunnel, Bridge, c)
	case "tcpTrans":
		service = proxy2.NewTunnelModeServer(proxy2.HandleTrans, Bridge, c)
	case "udp":
		service = proxy2.NewUdpModeServer(Bridge, c)
	case "webServer":
		InitFromCsv()
		t := &models.Tunnel{
			ServerPort: 0,
		...
		AddTask(t)
		service = proxy2.NewWebServer(Bridge)
