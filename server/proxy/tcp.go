...
func (s *TunnelModeServer) Start() error {
	return conn.NewTcpListenerAndProcess(s.task.ServerIp+":"+strconv.Itoa(s.task.Port), func(c net.Conn) {
		if err := s.CheckFlowAndConnNum(s.task.Client); err != nil {
			logs.Warn("%s:%d client:%d tcp connect error:%s", c.RemoteAddr(), s.task.Port, s.task.Client.Id, err.Error())
			c.Close()
			return
		}
		logs.Trace("%s:%d new tcp connection,client:%d", c.RemoteAddr(), s.task.Port, s.task.Client.Id)
...
func ProcessHttp(c *conn.Conn, s *TunnelModeServer) error {
	...
	if err := s.auth(r, c, s.task.Client.Cnf.User, s.task.Client.Cnf.Passwd); err != nil {
		return err
	}
