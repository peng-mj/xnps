...
func createNetwork(name, networok string) error {
	// docker network create --subnet=172.18.0.0/16 web_test
...
func runDocker(dockerName, networkName, ip, testFunName, nowDir string) error {
	// docker run --env GOPROXY=https://goproxy.cn  --rm --name client --net web_test --cap-add=NET_ADMIN --ip 172.18.0.5 -v "$PWD":/usr/src/myapp -w /usr/src/myapp golang go web_test -v -run TestClient ./
	return runCmd(exec.Command("docker", "run", "--env", "GOPROXY=https://goproxy.cn", "--rm", "--name", dockerName, "--net", networkName,
		"--cap-add=NET_ADMIN", "--ip", ip, "-v", nowDir+`:/usr/src/myapp`, "-w", `/usr/src/myapp`, "golang", "go", "web_test",
		"-v", "-run", testFunName, "./"))
}
