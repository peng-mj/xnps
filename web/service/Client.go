...
func (c *Client) GetAllClientList(start, length int64, search, sort, order string, clientId int) ([]models.Client, int) {
	...
	c.Orm(models.Client{}).Where("valid = 1").Find(&cli)
	return cli, len(cli)
}
...
func (c *Client) GetClientCountByMode(mode string) (count int64) {
	c.Orm(models.Client{}).Where("mode = ? ", mode).Count(&count)
	return
}
func (c *Client) UpdateClientById(client *models.Client, id int64) (count int64) {
	c.Orm(models.Client{}).Where("id = ?", id).Updates(client)
	return
}
func (c *Client) GetClientByAccessUser(AccessId, AccessKey string) (client models.Client, err error) {
	if c.Orm(models.Client{}).Where("access_id = ? and access_key = ?", AccessId, AccessKey).First(&client).RowsAffected == 0 {
...
func (c *Client) GetClientIdByVkey(vkey string) (id int64, err error) {
	...
	if c.Orm(models.Client{}).Where("verify_key = ?", vkey).First(&cli).RowsAffected < 1 {
	}
	return
}
...
func (c *Client) GetClientTunnelNumByClientId(id int64) int {
	var num int64
	c.Orm(models.Client{}).Where("client_id = ?", id).Count(&num)
	return int(num)
}
...
func (c *Client) VerifyUserName(username string, id int64) (res bool) {
	return c.Orm(models.Client{}).Where("web_user = ? AND id = ?", username, id).First(new(models.Client)).RowsAffected > 0
}
...
func (c *Client) CreateNewClient(client *models.Client) error {
	...
	if client.AccessKey == "" || isNotSet {
		isNotSet = true
		client.AccessKey = crypt.RandStr().Generate(40)
	}

	c.Orm(models.Client{}).Create(client)
	return nil
}
func (c *Client) DelClient(id int64) error {
	c.Orm(models.Client{}).Delete(&models.Client{Id: id})
	return nil
}
func (c *Client) VerifyClientVkey(vkey string, id int64) (res bool) {
	return c.Orm(models.Client{}).Where("verify_key = ? AND id = ?", vkey, id).First(new(models.Client)).RowsAffected > 0
}
